...
function createDictionaryEntry(title, dictUrl, entryId) {
    chrome.contextMenus.create({
        "title": title, "contexts": ["selection"], "id": entryId, "onclick": function (info) {
...
export function make_default_online_dicts() {
    ...
    if (uiLang != 'en' && Object.prototype.hasOwnProperty.call(isoLangs, uiLang)) {
        var langName = isoLangs[uiLang];
        result.push({ title: "Translate to " + langName + " in Google", url: "https://translate.google.com/#en/" + uiLang + "/" });
    }
    result.push({ title: "Define in weblio辞書", url: "https://www.weblio.jp/content/" });
    result.push({ title: "Define in goo辞書", url: "https://dictionary.goo.ne.jp/word/" });
