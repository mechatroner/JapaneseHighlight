    ...
    return new Promise((resolve) => {
        chrome.runtime.sendMessage({ text }, (response) => {
            ...
            for (let i = 0; i < lines.length - 2; ++i) {
                ...
                if (
                    !match &&
                    jhHlSettings.wordParams.enabled &&
                ...
                if (last_hl_end_pos < match.begin) {
                    ...
                    new_children.push(
                        document.createTextNode(
                            text.slice(last_hl_end_pos, match.begin)
                        )
                    )
                }
            ...
            if (insert_count && last_hl_end_pos < text.length) {
                new_children.push(
                    document.createTextNode(
                        text.slice(last_hl_end_pos, text.length)
                    )
                )
            }
...
async function doHighlightText(textNodes) {
    ...
    for (const textNode of textNodes) {
        ...
        const insert_count = await mecabQueue.add(() =>
            text_to_hl_nodes(text, new_children)
        )
...
function renderBubble() {
    ...
    bubbleFreq.style.backgroundColor = getHeatColorPoint(
        (rank / word_max_rank) * 100
    )
    ...
    bubbleDOM.style.left =
        Math.max(5, Math.floor((bcr.left + bcr.right) / 2) - 100) + 'px'
