...
function authorize_user(interactive_authorization) {
    chrome.identity.getAuthToken({ interactive: interactive_authorization }, token => {
        if (token === undefined) {
...
function substract_from_set(lhs_set, rhs_set) {
    for (var key in rhs_set) {
        if (Object.prototype.hasOwnProperty.call(rhs_set, key) && Object.prototype.hasOwnProperty.call(lhs_set, key)) {
            delete lhs_set[key];
        }
    }
}
...
function fetch_file_content(file_id, success_cb) {
    ...
    gapi.client.request({ 'path': full_query_url, 'method': 'GET' }).then((jsonResp) => {
        if (jsonResp.status != 200) {
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        if (request.wdm_request == "hostname") {
            ...
            if (request.wdm_verdict == "highlight") {
                ...
                chrome.browserAction.setIcon({ path: "../images/no_dynamic.png", tabId: sender.tab.id });
            } else {
                chrome.browserAction.setIcon({ path: "../images/result48_gray.png", tabId: sender.tab.id });
            }
